...
# Parzelle: Wird in diesem Script für "Zeilen" im Versuchsplan verwendet, also die horizontale Abfolge von Plots. Nicht verwechseln mit den Saatreihen, die einen Plot ausmachen.
# Reihe: Wird in diesem Script für "Spalten" im Versuchsplan verwendet, also die vertikale Abfolge von Plots.
...
# Wieviele Saatreihen einen Plot ausmachen sollen
...
setwd("U:\\Eigene Dateien\\4-Aussaaten GH+Feld\\Aussaat2024 Feld+GH\\BPGv_WG-phänotypische Bonituren\\fieldscript") 
...
Lines <- read.delim("WG-Linien zur Aussaat.txt",sep="\t",header=T, stringsAsFactors = F)
...
#### Größe und Form des Versuchs – Parzellen/Plots und Reihen
# In den folgenden Zeilen bitte eintragen, wie viele Parzellen und Reihen in einer Wiederholung gewünscht sind:
Parzellen <- 10 # Anzahl der Magazine pro Wiederholung (Anzahl der Parzellen) in einer Reihe
Reihen <-9 # Anzahl der Spalten pro Wiederholung ("Feldreihen/Spalten)

# Die Anzahl der Feldreihen/Spalten ergibt sich aus der Anzahl der Plots/Linien. Bsp.: Bei Parzellen mit 2 Linien in Doppelreihen sind bei 5 Feldreihen = 10 Feldreihen/Spalten einzutragen!
...
seeds<-c(seed_rep1 = 44254, seed_rep2 = 34523452)

#### Wiederholungen
# Hier die Anzahl der Wiederholungen festlegen. Bitte beachten, dass die Anzahl der Wiederholungen nicht höher als die Anzahl der Variablen im "seeds"-Vektor (s. oben) sein sollte.
Wiederholungen = 2

# Bitte festlegen, ob die Wiederholungen "nebeneinander" (entlang der "Reihen") oder "übereinander" ("entlang der "Parzellen") angeordnet werden sollen, also:
...
# Hier bitte angeben, wieviele Saatreihen einen Plot ergeben sollen, also z.B. ob Doppelreihen oder komplette Parzellen (6 Reihen/3 Doppelreihen) gewünscht sind. Bei Doppelreihen bitte Nr_Plotreihen = 2 setzen, bei ganzen Parzellen Nr_Plotreihen = 6, da sechs Saatreihen eine Parzelle ergeben. Es ist jede beliebige Nummer möglich, so auch 1, 3, 4 etc., was aber in der Praxis eher nicht gebraucht wird.
Nr_Plotreihen = 2
...
if (! is.numeric(Parzellen)){
  cat("Ungültige Anzahl der Parzellen Bitte numerischen Wert eingeben.")
  checklist<-c(checklist, F)
  } else {
  cat("Kontrolle - Wurde für Anzahl der Parzellen ein numerischer Wert eingegeben? Ok.")
  checklist<-c(checklist, T)
  }
...
if (! is.numeric(Nr_Plotreihen)){
  cat("Ungültige Anzahl der Saatreihen pro Plot. Bitte numerischen Wert eingeben.")
  checklist<-c(checklist, F)
  } else {
  cat("Kontrolle - Wurde für Anzahl der Saatreihen pro Plot ein numerischer Wert eingegeben? Ok.")
  checklist<-c(checklist, T)
  }


#Check if number of rows x number of columns = number of plots
if (Parzellen*Reihen != sum(Lines[2])){
  cat(paste0("Die Anzahl der Parzellen (", Parzellen, ") mal der Anzahl der Reihen (", Reihen, ") ergibt nicht die Anzahl der benötigten Plots (", sum(Lines[2]), "). Bitte korrigieren."))
  checklist<-c(checklist, F)
}else {
  cat(paste("Kontrolle - Ergibt die Anzahl der Parzellen mal der Anzahl der Reihen die Anzahl benötigter Plots?", Parzellen, " * ", Reihen, " = ", Parzellen*Reihen, ". Ok."))
  checklist<-c(checklist, T)
}
...
if (all(checklist)){
  ...
  cat("Eine oder mehrere Eingaben sind falsch. Bitte Eingaben in den Zeilen 134 bis 218 überprüfen.")
    }
...
#################################################
# Eigentliches Script - Erstellen der Feldpl¨äne #
#################################################
...
field_design=function(x, seeds){
  if (all(checklist)){ # Do a final check of all inputs
    ...
    for (i in seq_along(1:Wiederholungen)){
      ...
      layout_fieldplan <- matrix(book$PlotLine, nrow= Parzellen, ncol= Reihen, byrow = T)
      ...
      if (orientation == "vertical"){
        ...
        Row_offset <- (i * Parzellen) - Parzellen
    ...
    if (middle_rows == "yes"){
      ...
      if (orientation == "vertical"){
        fillermatrix<-matrix(middle_rows, nrow = Parzellen*Wiederholungen, ncol = length(fullsubvectortor),byrow = T)
        } else if (orientation == "horizontal"){
        fillermatrix<-matrix(middle_rows, nrow = Parzellen, ncol = length(fullsubvectortor),byrow = T)
        }
